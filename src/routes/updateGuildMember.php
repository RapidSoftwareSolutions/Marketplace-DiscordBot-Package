<?php
$app->post('/api/DiscordBot/updateGuildMember', function ($request, $response, $args) {
    $settings = $this->settings;

    //checking properly formed json
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['accessToken', 'guildId', 'userId']);
    ...
    if(isset($post_data['args']['moveToChannelId']) && strlen($post_data['args']['moveToChannelId']) > 0){
        $body['channel_id'] = $post_data['args']['moveToChannelId'];
    }
